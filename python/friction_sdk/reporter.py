...
class FrictionReporter:
    ...
    def __init__(
        ...
        # Auto mode: use remote if API key available, else local
        if self.output == "auto":
            self.output = "remote" if self.api_key else "local"
    ...
    def _send_remote(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        endpoint = os.getenv("TELNYX_FRICTION_ENDPOINT", self.DEFAULT_ENDPOINT)
